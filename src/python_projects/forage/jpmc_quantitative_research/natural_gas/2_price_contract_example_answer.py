    ...
    for i in range(len(all_dates)):
        ...
        if start_date in in_dates:
            ...
            if volume <= total_vol - rate:
                ...
                print(
                    "Injected gas on %s at a price of %s"
                    % (start_date, in_prices[in_dates.index(start_date)])
                )

            else:
                # We do not want to inject when rate is greater than total volume minus volume
                print(
                    "Injection is not possible on date %s as there is insufficient space in the storage facility"
                    % start_date
                )
            ...
            if volume >= rate:
                ...
                print(
                    "Extracted gas on %s at a price of %s"
                    % (start_date, out_prices[out_dates.index(start_date)])
                )
            else:
                # we cannot withdraw more gas than is actually stored
                print(
                    "Extraction is not possible on date %s as there is insufficient volume of gas stored"
                    % start_date
                )

    store_cost = (
        math.ceil((max(out_dates) - min(in_dates)).days // 30) * storage_cost_rate
    )
